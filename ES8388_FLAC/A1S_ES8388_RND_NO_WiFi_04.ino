...
#define CODEDIR "/Arduino/CODY/ES8388/A1S_ES8388_RND_NO_WiFi/A1S_ES8388_RND_NO_WiFi_04.ino"
#include "Arduino.h"
//#include "WiFi.h"
#include "SPI.h"
//#include "SD.h"
...
void setup(){
    Serial.begin(115200);
              ...
              for (int i = 0; i < test.size(); i++){
                ...
                pliki.clear();  
               // pliki.insert(pliki.begin(), "1.flac");
                //pliki.insert(pliki.begin(), "2.flac");
                pliki.push_back("1.flac");
                pliki.push_back("2.flac");
                pliki.push_back("3.flac");
                Serial.printf("num files %i\n", pliki.size());
                pliki.pop_back();
                const char* s0 = (const char*) pliki[0];
                const char* s1 = (const char*) pliki[1];
                const char* s2 = (const char*) pliki[2];
                Serial.print("s0=");Serial.println(s0);
                Serial.print("s1=");Serial.println(s1);
                Serial.print("s2=");Serial.println(s2);
                Serial.println("names.clear 0");
                names.clear();
                 Serial.println("names.clear 1");
                names.push_back("names---0");
                names.push_back("names---1");
                names.push_back("names---2");
                Serial.print("n0=");Serial.println(names[0]);
                Serial.print("n1=");Serial.println(names[1]);
                Serial.print("n2=");Serial.println(names[2]);
                for(int i = 0; i < names.size(); i++){
                  Serial.print(i);Serial.println(names[i]);
                    //Serial.printf("names%n=%s\n",i,names[i].c_str());
                }
  ...
  pinMode(KEY6, INPUT_PULLUP);
    ...
    readFile(SD, "/info.txt");
             ...
             Serial.print("File count on SD mp3: "); Serial.println(cnt);
             delay(1000);*/
...
void loop(){
    ...
    if (millis()-previousMillis > intervalTime){
        ...
        if (counterLoop % 150 == 0){
          ...
          Serial.printf("counter=%i nowTime=%i\n", counterLoop,nowTime);
        }
        esp_task_wdt_reset();
    }
